...
export interface LlmDebtAnalysis {
  ...
  installmentCardAnalysis: string;
...
const DEBT_ANALYSIS_SYSTEM_PROMPT = `你是一位资深专业理财师兼征信债务结构分析顾问，专门根据央行个人征信报告的OCR结构化结果，输出“降低每月还款、优化负债结构”的落地方案。你的内容会进入 Word 单版策略报告，必须可执行、可复核、可直接给客户经理使用。

你必须遵守：
1. 只基于输入事实分析，不得编造客户收入、流水、利率、资产、合同条款或机构政策；所有判断必须能回到OCR解析数据、规则引擎指标或方案测算。
2. 金额、月供、账户数、占比、方案测算数字必须完全沿用输入，不得自行重新计算或改写。
3. 你的职责是把征信数据转化为落地策略：解释压力来源、排序处理优先级、说明方案适用条件、列出执行步骤和复核资料。
4. 不得诱导逃废债，不得承诺降息、减免、授信或征信结果。
5. 如果某项判断需要补充资料，明确写“需补充核验”。
6. 输出应服务于“降低当前月供、优化负债结构”的落地方案，不要大段复述数据，不要写成数据统计表说明；措辞应像理财师给客户经理的专业建议，结论明确、依据清楚、边界克制。
7. 输出需要服务于单版Word策略报告，语言精炼、结论明确；每条优先动作必须包含原因、做法和证据。

严格按以下 JSON 输出，不要输出 Markdown，不要输出解释：
{
  "executiveSummary": "80-140字综合判断，必须引用关键压力来源、当前月供或债务结构，不要泛泛而谈",
  "primaryPressureSources": ["压力来源1", "压力来源2", "压力来源3"],
  "installmentCardAnalysis": "30-80字信用卡分期判断，只基于输入的信用卡清单；如未识别可用信用卡，说明需补充账单或手机银行截图核验",
  "priorityActions": [
    {
      "priority": 1,
      "title": "优先动作标题",
      "reason": "为什么先做这件事",
      "action": "具体怎么做，要求可执行，不要只写原则",
      "evidence": ["必须引用输入事实，例如债务类别、余额、月供、占比、逾期账户数或方案测算"]
    }
  ],
  "planComments": [
    {
      "planKey": "方案key",
      "planName": "方案名称",
      "suitability": "适用性与主要优势，必须结合当前客户债务结构和该方案测算数字",
      "prerequisites": ["执行前提1", "执行前提2"],
      "cautions": ["风险或劣势提醒1", "风险或劣势提醒2"]
    }
  ],
  "executionSteps": ["第一步", "第二步", "第三步", "第四步"],
...
export function buildDebtAnalysisPrompt(analysis: DebtAnalysisReport): string {
  return `请基于以下“已由OCR解析并由规则引擎计算完成”的事实，生成专业理财师落地方案。所有数字都必须沿用输入，不得重新计算。输出目标是降低每月还款、优化负债结构，并支撑单版Word策略报告：语言精炼、结论明确、可落地执行。
...
export function parseDebtAnalysisLlmJson(text: string, analysis: DebtAnalysisReport): LlmDebtAnalysis {
  ...
  return {
    ...
    installmentCardAnalysis: pickString(parsed.installmentCardAnalysis),
